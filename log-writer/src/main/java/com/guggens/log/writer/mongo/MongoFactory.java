...
public class MongoFactory {
     ...
     */
    @NotEmpty
    private List<ServerAddressBuilder> connections = new ArrayList<ServerAddressBuilder>();
     ...
     * @throws {@link UnknownHostException} Thrown if the server can not be found.
     * @throws {@link NoDBNameException} Throw in the db name is null.
     */
    public DB buildDB(Environment env) throws UnknownHostException, NoDBNameException {
        if (this.dbName == null)
            throw new NoDBNameException();
     ...
     * @throws {@link UnknownHostException} Thrown if the server can not be found.
     * @throws {@link NoDBNameException} Throw in the db name is null.
     * @throws {@link MissingMongoCollectionNameException} Thrown if the collection name is null.
     */
    public DBCollection buildColl(Environment env) throws UnknownHostException, NoDBNameException, MissingMongoCollectionNameException {
        if (this.dbName == null)
            throw new NoDBNameException();

        if (this.collName == null)
            throw new MissingMongoCollectionNameException();
    ...
    private List<ServerAddress> buildServerAddresses(List<ServerAddressBuilder> conns, Environment env) throws UnknownHostException {
        final List<ServerAddress> sal = new ArrayList<ServerAddress>(conns.size());
